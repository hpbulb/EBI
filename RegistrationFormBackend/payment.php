    ...
    try {
        ...
        if ($action === 'verify') {
            ...
            $payment = $pdo->prepare('SELECT * FROM application_payments WHERE reference = ?');
            ...
            $pdo->prepare("UPDATE application_payments SET status = 'paid', paid_at = COALESCE(paid_at, CURRENT_TIMESTAMP) WHERE reference = ?")->execute([$reference]);
        ...
        paymentResponse(false, 'Payment setup failed: ' . $e->getMessage(), 500);
    }
